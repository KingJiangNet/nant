...
namespace NAnt.Win32.Tasks
{
    ...
    internal class LocateSdkTask : Task
    {
        ...
        private readonly string _registryBaseWow6432SDKBase = @"SOFTWARE\Wow6432Node\Microsoft\Microsoft SDKs\";
        ...
        /// <summary>
        ///     <para>
        ///     The reg sub path unders windows sdks.
        ///     e.g. NETFXSDK\4.8
        ///     </para>
        /// </summary>
        [TaskAttribute("sdkregsubpath")]
        public string SdkRegSubpath
        {
            get { return _sdkRegSubpath; }
            set { _sdkRegSubpath = value; }
        }
        ...
        {
            if (!string.IsNullOrEmpty(SdkRegSubpath))
            {
                RegistryKey sdkRegSubKeyBase = Registry.LocalMachine.OpenSubKey(_registryBaseWow6432SDKBase, false);
                RegistryKey installedWinSdkSubKey = sdkRegSubKeyBase.OpenSubKey(SdkRegSubpath, false);

                if (installedWinSdkSubKey == null)
                    throw new Exception("'SdkRegSubpath' key in registry not found: " + SdkRegSubpath);

                object installDir = installedWinSdkSubKey.GetValue("InstallationFolder");

                if (installDir == null)
                    throw new Exception("'InstallationFolder' in registry not found in: " + SdkRegSubpath);

                Properties[_propName] = installDir.ToString();
                return;
            }
